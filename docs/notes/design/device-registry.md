...
The base URL is `control_plane::base_url()`: the compiled-in `https://teachouse.io`, which is the origin `tauri.conf.json`'s content-security policy names, overridable by `TAM_CONTROL_PLANE` for development.
