...
- date: 2026-09-03
- status: every tag from `v0.1.3` to `v0.3.4` is published to both the Cloud's beta channel and GitHub; from `v0.2.0` the Android job succeeds in the same run, so the APK and its sums file ride the release rather than a repair dispatch; `v0.4.0` is the release that follows each phase of the 2026-09-12 plan of record, and a release is now owed at every phase's end; Windows builds are still unsigned because no Azure Artifact Signing account exists
