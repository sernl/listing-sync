...
| `Category.Category` | `CategoryGroup.custom_categories` (`fields.rs:240`) | Expressed, with a stated gap: TPT takes integer ids and we hold seller strings, and no option set exists to seed the binding |
...
| `ItemDigital.thumb1` | `FileGroup.thumbnails[0]` (`fields.rs:131`) | Expressed in the model; the form renders the slot and collects no bytes (`tpt-form.ts:660-666`) |
...
| `ItemsCommonCoreStandard.common_core_standard_id` | `StandardAlignment.tpt_node_id` (`fields.rs:255`) | Expressed |
...
| `ItemsLocalization.country_id_flag` | none | Needs a new canonical field — see the kill gate below |
| `ItemsProperty.answer_key` | `DetailGroup.answer_key` (`fields.rs:264`) | Expressed |
| `ItemsProperty.copyright_declaration` | `TptBaseProduct.copyright` (`mod.rs:80`) | Expressed |
| `ItemsProperty.duration` | `DetailGroup.teaching_duration` (`fields.rs:261`) | Expressed |
| `ItemsProperty.pages` | `DetailGroup.pages_or_slides` (`fields.rs:263`) | Expressed |
...
| `TaxonomyTags` | `CategoryGroup.grades`, `.subject_areas`, `.tags`, `.formats` (`fields.rs:234-237`) | Expressed: four pickers serialise into one flat slug array |
...
Twenty-three paths are expressed today, one needs a new canonical field, and twenty-four are deliberate omissions, which sums to 48.
...
A grep for country or localisation across `crates/tam-domain/src/product/` and across `web/src/lib/tpt-form.ts` returns nothing, so neither the model nor the form holds it.
...
Categories gains the localisation checkbox as its last control, after Education Standards and before the section closes, which is where TPT puts it.
