...
Recorded 2026-09-04 with the deletion: `custody.rs` is trimmed rather than deleted, and the M1e custody guarantee is kept rather than retired. The file asserted two things — that `tam_app` cannot read `connection_secret` and that `tam_broker` can — and only the second goes, because the role goes. The first is the severity test the module doc names, it survives the broker whole, and it means more afterwards than before: `connection_secret` stays frozen and now has no reader at all, so what the file asserts is the application role's denial against a vault nothing opens.
...
9. Make D1's rule build-failing on the code rather than only on the declaration.
   The declaration is already gated in four places and a fifth would restate them: the transport-class test at `crates/tam-domain/src/registry/mod.rs:323`, the pair pinned in `crates/tam-types/src/lib.rs:836-914`, the column and its CHECK in `crates/tam-storage/migrations/0043_transport_class.sql`, and `every_marketplace_transport_class_matches_the_rust_source_of_truth` with `every_declared_marketplace_has_a_transport_class_row` in `crates/tam-storage/tests/leases.rs:2231` and `:2263`, which is what stops the column and the Rust function drifting apart.
   ...
   Half of that is now done, in S3's C4: `tam-import`'s operator binary sits behind a default-on `operator` feature, with the Tpt adapter, the pipeline, the secrets crate and the runtime optional under it and `required-features` on the `[[bin]]`.
   The reason was `tam-api` needing `import_one` for the device-import route and not being willing to gain a marketplace adapter to get it, so it depends with `default-features = false`; `tam-sync-worker` does the same, which leaves the binary's own `required-features` as the only thing that turns the feature on.
   Measured rather than asserted: `cargo tree -e normal -p tam-api -i tam-marketplace-tpt` and the same for `-tes` and for `-p tam-server` each answer `error: package ID specification ... did not match any packages`, which is cargo saying the package is absent from that graph entirely.
   What remains for the gate itself is `tam-canary`, which still builds a Tes adapter from a cookie-jar path, so the assertion can be written over the server binaries once question 3 disposes of it.
