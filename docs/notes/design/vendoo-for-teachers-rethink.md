...
| D31 | New. An organisation carries a slug beside the display name it already had -- 3 to 32 characters, lowercase letters, digits and single interior hyphens, unique case-insensitively under an index on `lower(slug)` -- and the seller chooses it at a first-run claim screen after the first successful sign-in rather than on the signup form. The departure from the signup form is mechanical: the verification email breaks the carriage between the form and the first session exchange, which is the only moment an organisation is created, and every place to park the value in between either breaks the `tam-auth` boundary or is lost when the seller finishes on another device. Existing organisations get `NULL` and no derived backfill, because a slug derived from the row's UUID is the provisional `org-{uuid}` name wearing a different hat. A newly provisioned organisation meets the screen as a hard gate before the console and one that predates the change meets a dismissable banner; nothing in the row tells them apart, so migration 0057 records the distinction once, at the one instant it is knowable. The slug is changeable, the old one is not reserved and no history is kept, because nothing embeds one today -- the first public URL that does converts this into a redirect obligation. "Changeable by the owner" is deliberately not claimed: there is no ownership or membership concept yet, so adding invites is the moment that question arrives rather than a moment at which it is silently answered. One bound in the change lives outside `crates/tam-limits`: the availability endpoint carries a per-session probe budget of thirty in a rolling minute, whose constant sits beside the route it governs, and it is an unadopted bound rather than an agreed one until the founder's next limits round adopts or removes it. | 2026-09-05 |
| D32 | New. A resource kept on Teachouse alone may carry no file at all, and a file becomes required the moment the resource is pointed at a marketplace -- for a draft there and for a live listing alike, so the rule is about the destination rather than the lifecycle state. This reverses the earlier decision that a product with no payload cannot be listed anywhere and should therefore be unrepresentable. That reasoning was sound about marketplaces and wrong about us: a seller works on a resource before deciding where it goes, and refusing to hold that work made Teachouse the one place the resource could not live. The invariant moves rather than disappearing -- `CanonicalProduct` carries an optional payload set and a set that exists is still non-empty by construction, migration 0061 relaxes `assert_product_has_payload` so it raises only for a product a mapping names, and a new deferred trigger on `mapping` refuses a mapping onto a product with no live payload. `POST /{version}/products` refuses a create naming an inventory with no payload, `POST /{version}/products/{product}/mappings` refuses a mapping onto a product with none, and `draft_refusals` raises `PayloadMissing` only where a marketplace is targeted, through one function the wasm core, the check endpoint and the create path all call. Deliberately not claimed: nothing about publishing a payload-less resource, which stays refused, and nothing about deleting the last file from a resource that already reaches a marketplace, which the mapping rule keeps refused. The migration is not reversible past this point -- relaxing the rule reverses cleanly, the payload-less products created afterwards do not -- and that is a runbook obligation rather than a decision. | 2026-09-05 |
| D33 | New. Best fit is pre-ticked on each marketplace tab of the authoring form, amending the founder's 2026-08-29 direction that it is an opt-in convenience and not the default. It holds on three conditions, each enforced rather than asserted: the tick never covers a `Delegation::Never` field, so the Tes licence and TPT's copyright declaration stay the seller's whatever it says; the tick writes a durable, revocable `ElectionRule` with `answer_kind = 'delegate'` per delegable axis, so the delegation is an auditable row rather than a browser preference and one call withdraws it; and a suggestion resolves only on explicit confirmation, because `resolved_by` settles nothing for a delegated answer. The ranking is over the resolved set and never the target's vocabulary, ordered by edge kind first and the seller's own order second. | 2026-09-05 |
