...
- TES delete is the documented `DELETE /api/v2/resources/{id}` for a published resource and the existing `/draft` delete for a draft, both verified by a follow-up read. TES update reuses the existing `set_metadata` step against an existing draft id. TES paid create is BLOCKED pending a founder capture of a paid TES create — the price wire format has never been observed and will not be invented.
...
- TES paid create: gated on a founder capture of a paid TES create. Owner: founder.
