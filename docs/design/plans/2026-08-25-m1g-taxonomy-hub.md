...
- Canonical term ids are deterministic: UUIDv5 under a new `NAMESPACE_TAM_TAXONOMY` over the name `tes:{gb_native_id}`.
  ...
  Corrected in flight from `tes:{mapTo}`: the captures show `mapTo` is not total — the Languages subject carries `[]` in both markets — while the GB native id is total over the seeding domain and durable, and `mapTo` is `9`+gb-id wherever it exists, so the two names agree up to a constant prefix; a dual scheme would mint a different id for Languages if Tes later populates its `mapTo`, which is exactly the drift a never-rotated namespace must not build in.
- The crosswalk pairs nodes on the measured rule itself — identical suffix (GB `1`+suffix, NZ `7`+suffix), byte-identical description, and paired parents — with `mapTo` demoted to a validation: non-empty on both sides it must equal `9`+gb-id, disagreement goes to the residue, and emptiness on either side blocks nothing.
