...
- Canonical term ids are deterministic: UUIDv5 under a new `NAMESPACE_TAM_TAXONOMY` over the name `tes:{gb_native_id}`.
  ...
  Corrected in flight from `tes:{mapTo}`, twice, and the second correction strengthened the first: the captures show `mapTo` is not total — the Languages subject carries `[]` in both markets — and not mechanical either, since 36 GB nodes (29 under Languages, 3 under Core IB, 2 under Anthropology, 2 subjects; Afrikaans 1100025 carries `91002025`) carry a `mapTo` that is not `9`+their own id and that names an id appearing nowhere in either tree.
  `mapTo` is a genuinely independent Tes scheme, so a name derived from it would drift against the node identity a never-rotated namespace must key on; the GB native id is total over the seeding domain, per-node unique, and the decision record's own durable-key convention.
- The crosswalk pairs nodes on the measured rule itself — identical suffix (GB `1`+suffix, NZ `7`+suffix, checked total and injective over both captures), byte-identical description, and paired parents — with `mapTo` demoted to a validation of side-to-side agreement: non-empty on both sides it must be equal across the markets (all 36 irregular values agree exactly), disagreement goes to the residue, and emptiness on either side blocks nothing.
  A node violating pairing goes to the residue with no edge on the unpaired side, so a drifted capture degrades to `Absent` plus a reconciliation item rather than a wrong edge.
...
- Discovered by the exhaustive round-trip test and adopted: a reverse-uniqueness withdrawal pass.
  Tes carries two distinct "Behaviour and classroom management" topics under "Whole school", so both canonical terms claim one target path as `Exact` in each market — precisely what the reverse-`Exact` unique index rejects at insert.
  The derivation withdraws every edge onto a contested path (both claimants, both markets; choosing a winner is the decision the design reserves for a human), records a `DuplicatePath` residue entry per withdrawn edge, and the two terms project `Absent` into the queue — the design's own stated failure mode.
