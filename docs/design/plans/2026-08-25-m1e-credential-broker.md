...
> **Superseded 2026-09-04 by founder decision D1.** This plan was implemented and its result has now been deleted: `crates/tam-session-broker` and the gateway lease are gone, because D1 puts every request to a marketplace with no official API on the seller's own device, which leaves no server-side session to hold a credential for. The `tam_broker` role outlives them as a name: a later migration revokes everything it holds and it stays inert in dev and CI, because three applied migrations grant to it and a replay cannot name a role that is not there; production drops it by the operator step in `../../notes/runbooks/retire-tam-broker-role.md`. The plan is kept for the reasoning it records — the custody seam, the AAD binding, the escrow requirement and the exculpability ordering — all of which decided where the architecture went rather than being abandoned with it. The one artefact that outlives the plan is `connection_secret`, which keeps the rows this plan's envelope sealed; disposing of them is a later founder call. Do not implement anything below.
