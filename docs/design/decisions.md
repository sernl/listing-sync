...
A HAR capture of the author dashboard settled both endpoints the plan deferred.
List-own-resources exists as a pair: `GET /api/v2/dashboard/getAllDrafts?page=N&limit=M` returns the author's drafts and `GET /api/v2/dashboard/getAllResources?page=N&limit=M` returns the published resources, each item carrying id, title, licence, price, the draft flag and the `/teaching-resource/...-{id}` url.
Own-file-download does not exist and this was confirmed by probing rather than assumed: the resource's `attachments` array carries only a signed CloudFront preview-image url and file metadata (name, mimeType, filesize, `isUploaded`), the CloudFront path serves only the preview image and refuses the original without Tes's own signature, and the attachment API returns metadata alone with no bytes and no signed original-file link.
The consequence is a product constraint rather than a capture gap: a GB-to-NZ duplication must re-upload the file bytes to build the NZ copy, and the server cannot obtain those bytes from Tes, so every migration requires the seller to supply the source files rather than the server fetching them from the marketplace.
The catalogue-listing legs can now be automated from the captured pair; the file-supply leg is the seller's, by the platform's own limitation.
