...
describe('the record a failed page leaves behind', () => {
	it('names the route and the error message', () => {
		const line = renderFailureReport(new Error('cover is not iterable'), '/inventory');
	...
	it('keeps a thrown string, which is a cause like any other', () => {
		expect(renderFailureReport('the vocabulary never arrived', '/inventory/new')).toContain(
