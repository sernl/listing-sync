...
import { MARKETPLACE_NAME, MARK_SRC, platformTitle } from '$lib/platforms';
...
export function inventoryMark(inventory: InventoryId): MarkView {
	if (!Object.hasOwn(MARKETPLACE_OF, inventory)) {
		return { src: null, name: platformTitle(inventory) };
	}
	return { src: MARK_SRC[MARKETPLACE_OF[inventory]], name: platformTitle(inventory) };
}
...
export function marketplaceMark(marketplace: Marketplace): MarkView {
	if (!Object.hasOwn(MARK_SRC, marketplace)) {
		return { src: null, name: marketplace };
	}
	return { src: MARK_SRC[marketplace], name: MARKETPLACE_NAME[marketplace] };
}
