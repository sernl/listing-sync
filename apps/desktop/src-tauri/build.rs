...
fn emit_entitlement_keys() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // `control_plane::base_url` reads this at compile time for a phone's
    // development build, so a change to it has to reach the crate.
    println!("cargo::rerun-if-env-changed=TAM_CONTROL_PLANE");
...
/// The capability a phone's development build needs to accept commands from
/// the control plane it was baked against.
///
/// `console.json` and `opener.json` grant the production origin and nothing
/// else, which is right: remote content is granted nothing until a capability
/// names it. A desktop `just desktop-dev` is not remote content — Tauri serves
/// the Vite origin as `devUrl`, so `local` covers it — but a phone has no dev
/// server of its own: `TAM_CONTROL_PLANE` bakes an origin in, and the page it
/// loads from there is remote in the capability sense. Without this file every
/// command on that build is refused with "not one the Teachouse app accepts
/// commands from", which is the production refusal doing its job on the wrong
/// build.
///
/// Written only when the variable is set, deleted when it is not, and
/// gitignored, so a release — which sets nothing — carries the two committed
/// files and no other. The permissions are the union of the two committed
/// capabilities, read from them rather than repeated, so a command added to
/// `console.json` reaches the phone without a second list to forget.
fn emit_dev_plane_capability() -> Result<(), Box<dyn std::error::Error>> {
    let path = std::path::Path::new("capabilities/dev-plane.json");
    let Some(origin) = supplied("TAM_CONTROL_PLANE") else {
        if path.exists() {
    ...
    for committed in ["capabilities/console.json", "capabilities/opener.json"] {
        let file: serde_json::Value =
            serde_json::from_reader(std::io::BufReader::new(std::fs::File::open(committed)?))?;
        let granted = file["permissions"]
            .as_array()
            .ok_or_else(|| format!("{committed} has no permissions list"))?;
        permissions.extend(granted.iter().cloned());
    }
    let capability = serde_json::json!({
        "$schema": "../gen/schemas/desktop-schema.json",
        "identifier": "dev-plane",
        "description": "Generated by build.rs from TAM_CONTROL_PLANE; a development build's console origin. Never committed.",
        "windows": ["main"],
        "remote": { "urls": [origin.trim_end_matches('/')] },
        "permissions": permissions,
    });
    std::fs::write(path, serde_json::to_vec_pretty(&capability)?)?;
    Ok(())
}
