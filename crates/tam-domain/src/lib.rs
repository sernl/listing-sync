...
///
/// Fails closed on anything it cannot read as exactly three dotted numbers.
/// A version this cannot parse is a client this code has never seen, and the
/// cost of guessing wrong in the permissive direction is the stuck lease the
/// gate exists to prevent, while guessing wrong in the refusing direction
/// costs a delay that ends at the device's next upgrade.
///
/// Deliberately not a semver dependency: the comparison is three numbers, the
/// only producer is our own installer, and a parser that accepted build
/// metadata and pre-release ordering would have more behaviour than the
/// question has.
#[must_use]
pub fn runs_sourced_payloads(app_version: &str) -> bool {
    let Some(triple) = parse_version_triple(app_version) else {
        return false;
    };
    triple >= SOURCED_PAYLOAD_MIN_VERSION
}

/// Exactly three dotted decimal numbers, or nothing.
///
/// Tuple comparison is what makes 0.10.0 sort above 0.9.0; comparing the
...
/// One numeric segment, refusing a spelling `u32::from_str` accepts and a
/// version does not.
///
/// A leading zero on a multi-digit segment is two spellings of one version,
/// and admitting both would let `00.2.0` and `0.2.0` be different strings
/// meaning the same thing in a comparison whose whole job is to be exact.
///
/// A leading `+` needs no guard here and never reaches this: the suffix split
/// above cuts at the first `-` or `+`, so `+0.2.0` arrives as an empty version
/// and `0.+2.0` as a two-segment one, and both are refused before a segment is
/// parsed. A guard here would read as the thing that rejects them and would
/// be dead.
fn segment(raw: &str) -> Option<u32> {
...
#[cfg(test)]
mod sourced_payload_gate {
    use super::{runs_sourced_payloads, SOURCED_PAYLOAD_MIN_VERSION};

    /// The ordering is numeric, which is the whole reason this is not a string
    /// comparison: 0.10.0 is above 0.9.0 and lexicographic ordering says the
    /// opposite. A build that compared strings would strand every device past
    /// 0.9 on the wrong side of the gate.
    #[test]
    ...
    fn anything_unparseable_reads_as_below_the_shim() {
        for raw in [
            ...
            assert!(
                ...
                "{raw:?} is not three dotted numbers and must fail closed"
            );
        }
    }
    ...
    fn the_shim_version_is_the_one_the_gate_compares_against() {
        ...
        assert!(
            ...
            "the constant's own spelling must pass the gate it defines"
        );
    }
}
