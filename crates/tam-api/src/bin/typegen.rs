...
fn vocab() -> String {
    ...
    out.push('\n');
    // What a thing costs money for, and the only billing vocabulary the
    // console may name: `POST /v1/billing/checkout` takes one of these, and
    // the server alone knows which Stripe price carries it.
    out.push_str(&closed("PriceKey", "PRICE_KEYS", &PriceKey::ALL, |key| {
        format!("\"{}\"", key.as_str())
    }));
    out.push('\n');
    out.push_str(&closed(
    ...
    // backoffice panel branch on: a purchased grant offers "Manage billing"
    // and an operator grant reads "Set by Teachouse until <date>". `paddle`
    // is still a value because the rows it wrote are still real grants.
...
fn plans_ts() -> String {
    let mut out = String::new();
    out.push_str("// Generated by tam-typegen. Do not edit; run `just web-typegen`.\n\n");
    out.push_str("import type { AiStatus, Plan, PriceKey, SupportLevel } from './vocab';\n\n");
    ...
    out.push_str("export const PACKS: readonly Pack[] = [\n");
    out.push_str(&pack_rows("  "));
    out.push_str("] as const;\n\n");
    out.push_str(&lit(
...
fn plans_js() -> String {
    ...
    out.push_str(&plan_rows("  "));
    out.push_str("]);\n\n");
    out.push_str("export const PACKS = Object.freeze([\n");
    out.push_str(&pack_rows("  "));
    out.push_str("]);\n\n");
    out.push_str(&lit(
        "export const PACK_ABOVE = ",
