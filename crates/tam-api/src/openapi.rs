...
pub const ROUTES: [Route; 175] = [
    ...
    Route {
        method: "post",
        path: "/{version}/migrations/plan",
        summary: "Preview a copy or a move: a verdict per resource, and the month's allowance",
    },
    Route {
        method: "post",
        path: "/{version}/migrations",
        summary: "Copy or move catalogue resources between two marketplaces; \
    ...
    Route {
        method: "patch",
        path: "/{version}/products/{product}/files/{file}",
        summary: "Rename one payload or preview file, leaving its bytes and role alone",
    },
    Route {
        method: "delete",
        path: "/{version}/products/{product}/files/{file}",
    ...
    Route {
        method: "post",
        path: "/{version}/devices/{device}/restore",
        summary: "Sign one device back in, from a session on that device",
    },
    Route {
        method: "post",
